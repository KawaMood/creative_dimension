...
function pk_cr_di:player/in_portal/leave

# Summon player's data marker
execute in overworld run summon marker -30000000 11 1600 {Tags:["pk_cr_di_player_data","pk_current_marker"]}

# Increment id and link player with the data marker
scoreboard players add %pk_cr_di_link_id PKDWId 1
scoreboard players operation @e[type=marker,tag=pk_current_marker,limit=1] PKDWId = %pk_cr_di_link_id PKDWId
scoreboard players operation @s PKDWId = %pk_cr_di_link_id PKDWId
...
data modify entity @e[type=marker,tag=pk_current_marker,limit=1] data.ActiveEffects set from entity @s ActiveEffects
...
execute as @e[type=marker,tag=pk_cr_di_portal,sort=nearest,limit=1] run function pk_cr_di:player/in_portal/store_entered_point
tag @s remove pk_current_player

# Teleport the player in the creative dimension
execute in pk_cr_di:creative_superflat run tp @s 0 1 0
execute at @s unless predicate pk_cr_di:blocks/two_blocks_height_air run spreadplayers ~ ~ 1 1 false @s
...
xp set @s 0 points
gamemode creative @s
tag @s add pk_cr_di_in_creative_dimension
